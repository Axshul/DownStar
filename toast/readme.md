...
Made with ❤️ by [Axshul](https://github.com/Axshul)
...
Use when you want to customize or debug the internal files.

```html
<script src="https://axshul.github.io/DownStar/toast/Call.js"></script>
<script>
  Toast.success("DownStar Toast (Development Mode)");
</script>
2. Production Mode
Use the pre-built single-file bundle for faster loading and optimized delivery.

html
Copy code
<script src="https://axshul.github.io/DownStar/toast/Toast.bundle.js"></script>
<script>
  Toast.success("DownStar Toast (Production Mode)");
</script>
3. CDN Mode (via jsDelivr)
Use globally cached and versioned assets through jsDelivr.
Tag versions for stable builds.

html
Copy code
<script src="https://cdn.jsdelivr.net/gh/Axshul/DownStar@v1.0.0/toast/Toast.bundle.js"></script>
<script>
  Toast.success("Loaded from CDN");
</script>
Toast API Reference
...
options (object) – Optional configuration overrides (see Configuration).

Example:

js
Copy code
Toast.show("Simple notification");
Toast.success(message, options)
Displays a success toast (green background).

js
Copy code
Toast.success("Data saved successfully");
Toast.error(message, options)
Displays an error toast (red background).

js
Copy code
Toast.error("An error occurred while processing");
Toast.warning(message, options)
Displays a warning toast (yellow background).

js
Copy code
Toast.warning("Check your inputs");
Toast.info(message, options)
Displays an informational toast (blue background).

js
Copy code
Toast.info("Welcome to DownStar");
Toast.clearAll()
...
Updates the global configuration for all new toasts.
...
position	string	"top-right"	Controls where toasts appear (top-left, top-right, bottom-left, bottom-right, top-center, bottom-center).
duration	number	4000	Time (in ms) before the toast auto-dismisses.
maxToasts	number	5	Maximum number of active toasts. Oldest toasts are removed first.
newestOnTop	boolean	true	Display new toasts on top of the stack.
pauseOnHover	boolean	true	Pause the timer when hovering over a toast.
showClose	boolean	true	Show or hide the close (“×”) button.
animation	string	"slide"	Toast animation style (fade, slide, scale).
theme	string	"dark"	Visual theme (dark, light, or custom).
offsetX	string	"20px"	Horizontal offset from the screen edge.
offsetY	string	"20px"	Vertical offset from the screen edge.
...
<head>
  <meta charset="UTF-8" />
  <title>DownStar Toast Example</title>
  <script src="https://axshul.github.io/DownStar/toast/Call.js"></script>
</head>
<body>
...
Built for both development and production usage.

Easily hosted and distributed via GitHub Pages + jsDelivr.

Simple API for instant use, flexible enough for customization.

Supports global configuration, animations, and themes.

Version History
Version	Date	Notes
1.0.0	Nov 2025	Initial release with hybrid architecture (Logic + Styles + Call + Bundle).
